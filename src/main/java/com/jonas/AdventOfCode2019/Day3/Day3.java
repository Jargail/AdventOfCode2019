package com.jonas.AdventOfCode2019.Day3;
...
public class Day3 {
            ...
            for (int i = 0; i < steps; ++i) {
                ...
                switch (direction) {
                    ...
                    case "L":
                        ...
                        break;
                    case "U":
                        pair = new Pair<>(currentPair.getKey(), currentPair.getValue() + 1);
                        path.add(pair);
                        currentPair = pair;
                        break;
                    case "D":
    ...
    private List<Pair<Integer, Integer>> findIntersection(
            List<Pair<Integer, Integer>> lst1, List<Pair<Integer, Integer>> lst2) {

        List<Pair<Integer, Integer>> lstRet = new ArrayList<>();
        for (Pair<Integer, Integer> pair1 : lst1) {
            for (Pair<Integer, Integer> pair2 : lst2) {
                if (pair1.getKey().intValue() == pair2.getKey().intValue()
                        && pair1.getValue().intValue() == pair2.getValue().intValue())
                    lstRet.add(pair1);
            }
        }
        return lstRet;
    ...
    private Integer findShortestPath(List<Pair<Integer, Integer>> lst) {
        Integer ret = null;
        for(Pair<Integer, Integer> p : lst) {
            if (ret == null || abs(p.getKey()) + abs(p.getValue()) < ret) {
                ret = abs(p.getKey()) + abs(p.getValue());
            }
        }
        return ret;
    }

    private class IntersectionSteps {
    ...
    private List<IntersectionSteps> findStepsToIntersection(
            List<Pair<Integer, Integer>> path1,
            List<Pair<Integer, Integer>> path2,
            List<Pair<Integer, Integer>> intersections) {

        List<IntersectionSteps> lstRet = new ArrayList<>();
        for (Pair<Integer, Integer> intersection : intersections) {
            IntersectionSteps steps = new IntersectionSteps();
            for (int i=0; i < path1.size(); ++i) {
                if (intersection.getKey().intValue() == path1.get(i).getKey().intValue()
                && intersection.getValue().intValue() == path1.get(i).getValue().intValue()) {
                    steps.pair = path1.get(i);
                    steps.steps1 = i+1;
                    break;
                }
            }
            ...
            for (int j=0; j < path2.size(); ++j) {
                if (intersection.getKey().intValue() == path2.get(j).getKey().intValue()
                        && intersection.getValue().intValue() == path2.get(j).getValue().intValue()) {
                    steps.steps2 = j+1;
                    break;
                }
            }
    ...
    private int findFewestSteps(List<IntersectionSteps> lst) {
        int ret = 0;
        for (IntersectionSteps steps : lst) {
            if (ret == 0 ||
            steps.steps1.intValue() + steps.steps2.intValue() < ret)
                ret = steps.steps1.intValue() + steps.steps2.intValue();
        }
        return ret;
    }
}
